...
export async function exportSummaryToPDF(
  ...
  logger.debug('Début de la fonction exportSummaryToPDF avec:', {
  ...
  try {
    ...
    logger.debug('HTML généré avec succès, longueur:', htmlContent.length);
    ...
    // Télécharger le PDF
    logger.debug('Téléchargement du fichier PDF:', fileName);
    const pdfBlob = doc.output('blob');
    saveAs(pdfBlob, fileName);
    logger.debug('Fichier PDF téléchargé avec succès');
    
    return Promise.resolve();
  } catch (error) {
    logger.error('Erreur lors de l\'exportation du compte rendu en PDF:', error);
...
export async function exportActionsToPDF(
  ...
  logger.debug('Début de la fonction exportActionsToPDF avec:', {
    summaryTextLength: summaryText.length,
    meetingName,
    meetingDate
  });
  ...
  try {
    ...
    logger.debug(`${actions.length} actions extraites du compte rendu`);
    ...
    logger.error('Erreur lors de l\'exportation des actions en PDF:', error);
    return Promise.reject(error);
  }
}
