...
export async function getClients(): Promise<Client[]> {
  try {
    logger.debug('Fetching available clients');
...
export async function associateMeetingWithClient(meetingId: string, clientId: string | null): Promise<any> {
  try {
    logger.debug(`Associating meeting ${meetingId} with client ${clientId || 'default template'}`);
    ...
    logger.debug('Meeting association updated successfully');
    return response;
  } catch (error) {
    logger.error(`Error associating meeting ${meetingId} with client:`, error);
    throw error;
  }
}
