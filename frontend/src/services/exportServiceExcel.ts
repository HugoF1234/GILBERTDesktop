...
export async function exportActionsToExcel(
  ...
  logger.debug('Début de la fonction exportActionsToExcel avec:', {
  ...
  try {
    ...
    logger.debug('Téléchargement du fichier Excel:', fileName);
    saveAs(blob, fileName);
    logger.debug('Fichier Excel téléchargé avec succès');
    
    return Promise.resolve();
  } catch (error) {
    logger.error('Erreur lors de l\'exportation des actions:', error);
