...
export async function exportSummaryToPDF(
  ...
  logger.debug('Début de l\'exportation du compte rendu au format PDF:', {
  ...
  try {
    ...
    logger.debug('Exportation PDF terminée avec succès');
  } catch (error) {
...
export async function exportSummaryToMarkdown(
  ...
  logger.debug('Début de l\'exportation du compte rendu au format Markdown:', {
    summaryTextLength: summaryText.length,
    meetingName,
    meetingDate
  });
  ...
  try {
    ...
    logger.debug('Exportation Markdown terminée avec succès');
  } catch (error) {
    if ((error as Error).message === 'Sauvegarde annulée') {
      logger.debug('Export annulé par l\'utilisateur');
      return;
    }
    logger.error('Erreur lors de l\'exportation du compte rendu en Markdown:', error);
    throw error;
  }
}
