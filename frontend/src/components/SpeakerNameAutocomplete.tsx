...
const SpeakerNameAutocomplete: React.FC<SpeakerNameAutocompleteProps> = ({
  ...
  // Synchroniser avec la valeur externe SEULEMENT si l'utilisateur n'est pas en train de taper
  useEffect(() => {
  ...
  return (
    <Autocomplete
      ...
      onInputChange={(_event, newInputValue, reason) => {
        ...
        // Si l'utilisateur tape librement (pas de sélection), mettre à jour la valeur
        if (reason === 'input') {
      ...
      renderInput={(params) => (
        <TextField
          ...
          onBlur={(event) => {
            // Reset typing state
            setIsUserTyping(false);
            // S'assurer que la valeur libre est bien sauvegardée lors du blur
          ...
          onKeyDown={(event) => {
            ...
            if (event.key === 'Enter') {
              event.preventDefault(); // Empêcher la soumission de formulaire
              setIsUserTyping(false);
              // S'assurer que la valeur libre est bien sauvegardée (utiliser inputValue car currentTarget n'a pas .value)
              if (inputValue !== value) {
                onChange(inputValue);
              }
              if (onKeyPress) onKeyPress(event as unknown as React.KeyboardEvent);
