...
// import OrganizationsView from './OrganizationsView'; // Remplacé par SharesView
import SharesView from './SharesView';
import ProfilePage from './ProfilePage';
import SettingsPage from './SettingsPage';
// Pages d'email supprimées - maintenant en amont dans App.tsx
import {
  Box,
  Paper,
  Typography,
...
interface MainContentProps {
  ...
  currentView: 'dashboard' | 'meetings' | 'templates' | 'admin' | 'user-details' | 'shares' | 'profile' | 'settings';
  ...
  isRecording?: boolean; // État d'enregistrement depuis App.tsx
  // organizationNavigationIntent?: OrganizationsNavigationIntent | null; // Désactivé
  // onOrganizationNavigationConsumed?: () => void; // Désactivé
}
...
const TranscriptionView = () => {
  ...
  const generateSummary = async (type: 'general' | 'commercial' | 'technical' | 'custom') => {
    ...
    logger.debug(`generateSummary appelé avec le type: ${type}`);
    
    const userConfirmed = window.confirm(`Voulez-vous vraiment générer un compte rendu de type ${type}?`);
    
    if (!userConfirmed) {
      logger.debug(`Génération du compte rendu de type ${type} annulée par l'utilisateur`);
      return;
    }
    
    logger.debug(`Génération du compte rendu de type ${type} confirmée par l'utilisateur`);
...
const MainContent: React.FC<MainContentProps> = ({
  ...
  onViewUserDetails,
  selectedUserId,
  // organizationNavigationIntent, // Désactivé
  // onOrganizationNavigationConsumed, // Désactivé
}) => {
  return (
    <Box sx={{ 
      flexGrow: 1, 
      ...
      p: currentView === 'templates' || currentView === 'shares' || currentView === 'profile' || currentView === 'settings' ? 0 : { xs: 2, md: 3 },
    }}>
      <Box sx={{ 
        minHeight: '100%',
        pb: currentView === 'templates' || currentView === 'shares' || currentView === 'profile' || currentView === 'settings' ? 0 : { xs: 4, md: 5 },
    }}>
      ...
      {currentView === 'templates' && <MyTemplates user={currentUser} />}
      {currentView === 'shares' && <SharesView user={currentUser} isMobile={isMobile} />}
      {currentView === 'admin' && <AdminDashboard onViewUserDetails={onViewUserDetails} />}
      {currentView === 'user-details' && selectedUserId && <UserDetailsPage userId={selectedUserId} />}
